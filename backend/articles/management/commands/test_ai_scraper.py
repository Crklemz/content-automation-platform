...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--compare',
            action='store_true',
            help='Compare AI scraper with traditional scraper'
        )
    ...
    def handle(self, *args, **options):
        ...
        # Compare with traditional scraper if requested
        if compare:
            self.stdout.write('\n3. Comparing AI scraper with traditional scraper:')
            traditional_scraper = NewsScraper()
            traditional_topics = traditional_scraper.get_trending_topics(category=category, limit=limit)
            
            self.stdout.write('\n   Traditional Scraper Results:')
            for i, topic in enumerate(traditional_topics, 1):
                self.stdout.write(f'   {i}. {topic.get("title", "No title")}')
                self.stdout.write(f'      Source: {topic.get("source", "Unknown")}')
            ...
            for i, topic in enumerate(ai_topics, 1):
                self.stdout.write(f'   {i}. {topic.get("title", "No title")}')
                self.stdout.write(f'      Source: {topic.get("source", "Unknown")}')
                self.stdout.write(f'      Quality: {topic.get("quality_score", 0):.2f}')
                self.stdout.write(f'      Sentiment: {topic.get("sentiment", "unknown")}')
                self.stdout.write(f'      Keywords: {", ".join(topic.get("keywords", [])[:3])}')
